...
def review_sample_path(
    *, split: str = DEFAULT_REVIEW_SPLIT, reviewed_dir: Path | None = None
) -> Path:
...
def held_out_eval_path(
    *, split: str = DEFAULT_REVIEW_SPLIT, reviewed_dir: Path | None = None
    ...
    name = (
        "held_out_eval.jsonl"
        if resolved_split == "dev"
        else f"held_out_eval_{resolved_split}.jsonl"
    )
    return resolved_dir / name


def held_out_eval_manifest_path(
    *, split: str = DEFAULT_REVIEW_SPLIT, reviewed_dir: Path | None = None
) -> Path:
    resolved_dir = reviewed_dir or (PATHS.data_benchmark / "reviewed")
    resolved_split = normalize_review_split(split)
    name = (
        "held_out_eval_manifest.json"
        if resolved_split == "dev"
        else f"held_out_eval_manifest_{resolved_split}.json"
    )
    return resolved_dir / name
