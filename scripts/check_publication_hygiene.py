...
def load_config(root: Path) -> tuple[set[str], set[str], dict[str, str]]:
    ...
    homes = ALLOWED_HOME | {item.lower() for item in config.get("allowed_home_segments", [])}
    emails = ALLOWED_EMAILS | {item.lower() for item in config.get("allowed_emails", [])}
    ...
    for relative in paths:
        ...
        if relative in pinned:
            if hashlib.sha256(data).hexdigest() != pinned[relative]:
                problems.append(f"{relative}: pinned historical file changed; review and repin")
            continue
        if b"\0" in data[:4096]:
            continue
        try:
...
def verify_detector() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        subprocess.run(["git", "init", "-q"], cwd=scratch, env=clean_git_env(), check=True)
        sample = scratch / "sample.txt"
        sample.write_text(good)
        subprocess.run(["git", "add", "sample.txt"], cwd=scratch, env=clean_git_env(), check=True)
        ...
        (scratch / CONFIG_NAME).write_text(
            json.dumps({"pinned_files": {"sample.txt": hashlib.sha256(good.encode()).hexdigest()}})
        )
        subprocess.run(["git", "add", CONFIG_NAME], cwd=scratch, env=clean_git_env(), check=True)
        assert scan(scratch)[0] == []
        sample.write_text(bad)
        subprocess.run(["git", "add", "sample.txt"], cwd=scratch, env=clean_git_env(), check=True)
        assert any("pinned historical file changed" in item for item in scan(scratch)[0])
